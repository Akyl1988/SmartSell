...
async def test_idempotency_reserve_and_replay(async_db_session: AsyncSession):
    ...
    allowed, status = await enforcer.reserve(async_db_session, company_id=1, key="abc", ttl_seconds=60)
...
async def test_idempotency_tenant_scoped(async_db_session: AsyncSession):
    ...
    allowed, _ = await enforcer.reserve(async_db_session, company_id=1, key="shared", ttl_seconds=60)
    assert allowed is True

    allowed, status = await enforcer.reserve(async_db_session, company_id=2, key="shared", ttl_seconds=60)
    assert allowed is True
    assert status is None
