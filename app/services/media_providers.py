...
class MediaProviderResolver:
    _cache: dict[tuple[str, int, str], MediaProvider] = {}
    _lock = asyncio.Lock()
    ...
    def _build_provider(
        ...
        if normalized.startswith("noop"):
            return NoOpMediaProvider(name=name, config=config, version=version)
        if normalized in {"cloudinary", "media-cloudinary"}:
            return CloudinaryMediaProvider(name=name, config=config, version=version)

        log.warning("Unknown media provider '%s', using noop", name)
        return NoOpMediaProvider(name=name, config=config, version=version)

    @classmethod
    async def resolve(cls, db: Any, *, domain: str = "media") -> MediaProvider:
        domain_key = (domain or "media").strip().lower() or "media"

        try:
            entry = await ProviderRegistry.get_active_provider(db, domain_key)
        except Exception as exc:  # pragma: no cover - runtime guard
            log.warning("Media provider resolution failed; using fallback noop", exc_info=exc)
            if settings.is_production:
                raise ProviderNotConfiguredError("media_provider_not_configured")
            fallback = NoOpMediaProvider(name="noop", config={}, version=0)
        ...
        if not entry:
            if settings.is_production:
                raise ProviderNotConfiguredError("media_provider_not_configured")
            log.warning("Media provider not configured for domain '%s'; using noop", domain_key)
            instance = NoOpMediaProvider(name="noop", config={}, version=0)
            async with cls._lock:
        ...
        if not provider_name.lower().startswith("noop"):
            try:
                ...
                log.warning("Media provider config fetch failed; using noop", exc_info=exc)
                provider_config = {}
        ...
        try:
            ...
            log.warning("Media provider build failed; using noop", exc_info=exc)
            await ProviderConfigService.record_event(
                db,
                domain=domain_key,
                provider=entry.provider,
