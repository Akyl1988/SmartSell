...
class WalletAccountCreate(WalletAccountBase):
    ...
    balance: Decimal | None = Field(default=Decimal("0"), description="Начальный баланс")
...
class WalletTransactionOut(WalletTransactionBase):
    ...
    @field_validator("currency", mode="before")
    @classmethod
    def _currency_norm_optional(cls, v: Any) -> str | None:
        if v is None:
            return None
        return _normalize_currency(v)
...
class LedgerItem(BaseModel):
    ...
    payment_id: int | None = None  # если операция связана с платёжкой
    ...
    @field_validator("currency", mode="before")
    @classmethod
    def _currency_norm_ledger(cls, v: Any) -> str | None:
        if v is None:
            return None
        return _normalize_currency(v)
