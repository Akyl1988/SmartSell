...
def _get_payment_storage(db: Session):
    try:
...
def _get_wallet_storage(db: Session):
    try:
...
async def create_and_capture(
    ...
    try:
        ...
        acc = await _ensure_account_access(req.wallet_account_id, current_user, db, wallet_storage)
