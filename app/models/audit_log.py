...
class AuditLog(BaseModel, TenantMixin, AuditMixin):
    ...
    source: Mapped[str | None] = mapped_column(String(64), index=True)  # api/worker/webhook…
    ...
    @classmethod
    def safe_log(cls, session: Session | None, action: str, **kwargs) -> AuditLog | None:
    ...
    # --------------------- Поиски/фильтры ---------------------
    @classmethod
    def get_by_id(cls, session: Session, log_id: int) -> AuditLog | None:
...
class AuditAction:
    ...
    @classmethod
    def get_action_doc(cls, action: str) -> str | None:
