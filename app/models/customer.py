...
class Customer(BaseModel, TenantMixin, SoftDeleteMixin, AuditMixin):
    ...
    @validates("phone")
    def _v_phone(self, _k: str, v: str | None) -> str | None:
        if v is None:
            return None
        return v.strip()

    @validates("full_name")
    def _v_fullname(self, _k: str, v: str | None) -> str | None:
    ...
    # ----------------------------- Query helpers ---------------------------- #
    @classmethod
    def find_by_email(cls, session: Session, email: str) -> Customer | None:
        if not email:
            return None
        return session.query(cls).filter(func.lower(cls.email) == email.strip().lower()).first()

    @classmethod
    def find_by_phone(cls, session: Session, phone: str) -> Customer | None:
        if not phone:
            return None
