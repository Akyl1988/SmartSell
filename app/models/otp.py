...
_AUDIT_SINK: Callable[[str, dict[str, Any]], None] | None = None


def configure_audit_sink(sink: Callable[[str, dict[str, Any]], None] | None) -> None:
    """Регистрирует внешний sink для аудита (логер/шина и т.п.)."""
    global _AUDIT_SINK
    _AUDIT_SINK = sink


def get_audit_sink() -> Callable[[str, dict[str, Any]], None] | None:
    return _AUDIT_SINK
...
if _existing_otpcode is None:
    ...
    class OTPCode(Base):
        ...
        # ---- Валидации ----
        @validates("phone")
        def _v_phone(self, _k: str, v: str | None) -> str:
            v = (v or "").strip()
            if not v:
        ...
        @validates("code")
        def _v_code(self, _k: str, v: str | None) -> str:
            v = (v or "").strip()
            if not v:
        ...
        @validates("purpose")
        def _v_purpose(self, _k: str, v: str | None) -> str:
            return (v or "login").strip().lower()

        @validates("expires_at")
        def _v_expires(self, _k: str, v: datetime | None) -> datetime:
...
class OtpAttempt(Base):
    ...
    @validates("channel")
    def _v_channel(self, _k: str, v: str | None) -> str:
    ...
    def bump_fraud(self, points: int, *, flag: str | None = None) -> int:
    ...
    # Rate limiting
    def _reset_hour_if_needed(self, now: datetime | None = None) -> None:
        now = now or utc_now()
    ...
    def create_new(
        ...
        phone: str,
        code_hash: str,
        purpose: str | OtpPurpose = OtpPurpose.LOGIN.value,
        expires_minutes: int = DEFAULT_OTP_TTL_MIN,
        user_id: int | None = None,
        channel: str | OtpChannel = OtpChannel.SMS.value,
        attempts_left: int = 5,
    ) -> OtpAttempt:
        now = utc_now()
    ...
    def emit_audit_event(self, event_name: str, *, extra: dict[str, Any] | None = None) -> None:
        _emit(event_name, self.to_audit_dict(extra=extra))

    def to_audit_dict(self, *, extra: dict[str, Any] | None = None) -> dict[str, Any]:
        payload = {
            "id": self.id,
            "phone": self.phone,
