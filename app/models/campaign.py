...
class Campaign(SoftDeleteMixin, Base):
    ...
    def bulk_add_messages(
        ...
        *,
        ...
        template_vars_iter: Iterable[dict[str, Any]] | None = None,
        channel: ChannelType = ChannelType.EMAIL,
    ) -> list[Message]:
        msgs: list[Message] = []
...
class Message(SoftDeleteMixin, Base):
    ...
    def mark_sent(self, *, provider_id: str | None = None, when: datetime | None = None) -> None:
        if self.is_deleted:
    ...
    def mark_delivered(self, *, when: datetime | None = None) -> None:
        if self.is_deleted:
            raise ValueError("Нельзя изменять удалённое сообщение.")
