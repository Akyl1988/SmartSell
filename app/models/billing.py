...
def to_astana(dt: datetime | None) -> datetime | None:
    if not dt:
        return None
    try:
...
def _norm_currency_code(v: str | None, *, default: str = "KZT") -> str | None:
    vv = (v or "").strip().upper()
...
class BillingPayment(BaseModel, SoftDeleteMixin):
    ...
    provider_receipt_url: Mapped[str | None] = mapped_column(String(1024))

    authorized_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    ...
    company: Mapped[Any | None] = relationship("Company", back_populates="billing_payments")
    ...
    # -------- нормализация --------
    @validates("currency")
    def _norm_currency(self, _k: str, v: str | None) -> str | None:
        return _norm_currency_code(v, default="KZT")

    @validates("status")
    def _norm_status(self, _k: str, v: str | None) -> str | None:
        vv = (v or "").strip().lower()
    ...
    @validates("method")
    def _norm_method(self, _k: str, v: str | None) -> str | None:
        vv = (v or "").strip().lower()
    ...
    @validates("provider")
    def _strip_provider(self, _k: str, v: str | None) -> str | None:
    ...
    def mark_authorized(self, *, at: datetime | None = None) -> None:
        self.status = PaymentStatus.AUTHORIZED.value
        self.authorized_at = at or utc_now()

    def mark_captured(self, *, at: datetime | None = None) -> None:
        self.status = PaymentStatus.CAPTURED.value
        self.captured_at = at or utc_now()

    def mark_partial(self, *, at: datetime | None = None) -> None:
        self.status = PaymentStatus.PARTIAL.value
        self.captured_at = at or utc_now()

    def mark_refunded(self, *, at: datetime | None = None) -> None:
        self.status = PaymentStatus.REFUNDED.value
        self.refunded_at = at or utc_now()

    def mark_failed(self, *, at: datetime | None = None, desc: str | None = None) -> None:
    ...
    def mark_voided(self, *, at: datetime | None = None, desc: str | None = None) -> None:
        self.status = PaymentStatus.VOIDED.value
        self.failed_at = at or utc_now()
        if desc:
            self.description = f"{(self.description or '').strip()} | voided: {desc}".strip(" |")

    def link_receipt(self, url: str | None) -> None:
    ...
    def maybe_affect_order_status(self, session: Session, *, set_paid_on_capture: bool = True) -> None:
        ...
        if not self.order_id:
            return
        order: Order | None = session.get(Order, self.order_id)
        if not order:
            return
        if (
            set_paid_on_capture
    ...
    def export_query(
        *,
        ...
        method_in: Sequence[str] | None = None,
...
class Subscription(BaseModel, SoftDeleteMixin):
    ...
    company: Mapped[Any | None] = relationship("Company", back_populates="subscriptions")
    ...
    def _strip_val(self, _k: str, v: str | None) -> str | None:
        return (v or "").strip() or None
    ...
    def grace_expires_at(self) -> datetime | None:
    ...
    def schedule_next_billing(self, *, from_date: datetime | None = None) -> None:
...
class Invoice(BaseModel, SoftDeleteMixin):
    ...
    def link_pdf(self, url: str | None = None, path: str | None = None) -> None:
    ...
    def factory(
        ...
        *,
        ...
        status: str = "unpaid",
        subtotal: Decimal | int | float | None = None,
...
class BillingInvoice(BaseModel, SoftDeleteMixin):
    ...
    def apply_payment(self, amount: Decimal | int | float, *, when: datetime | None = None) -> None:
...
class WalletBalance(BaseModel, SoftDeleteMixin):
    ...
    def _auto_topup_deficit_via_gateway(
        self, *, session: Session, deficit: Decimal, idempotency_key: str | None = None
    ) -> None:
        if deficit <= 0:
            return
    ...
    async def _auto_topup_deficit_via_gateway_async(
        self, *, session: AsyncSession, deficit: Decimal, idempotency_key: str | None = None
    ) -> None:
        if deficit <= 0:
            return
        if (self.credit_limit or Decimal("0")) < deficit:
    ...
    def maybe_autotopup(
        ...
        *,
        ...
        reference_type: str | None = "autotopup",
        reference_id: int | None = None,
    ...
    def settle_amount_with_wallet(
        ...
        *,
        ...
        invoice_internal_notes: str | None = None,
...
async def _emit_audit_safe_async(
    *, session: AsyncSession, company_id: int, action: str, meta: dict[str, Any] | None = None
) -> None:
    try:
        from app.models.audit import ExternalAuditEvent  # type: ignore
    except Exception:
...
def _within_range(dt: datetime | None, df: datetime | None, dtmax: datetime | None) -> bool:
    if not dt:
        return True
    if df and dt < df:
        return False
...
def operations_feed_sync(
    ...
    *,
    ...
    date_from: datetime | None = None,
    date_to: datetime | None = None,
    kinds: Iterable[OperationKind] | None = None,
    limit: int = 500,
) -> list[dict[str, Any]]:
    """
