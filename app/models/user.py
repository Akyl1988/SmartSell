...
class User(
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "role IN ('admin','manager','storekeeper','analyst','platform_admin','platform_manager')",
    ...
    @validates("email")
    def _validate_email(self, _key, value: str | None):
        if value is None:
            return value
        v = value.lower().strip()
    ...
    @validates("username")
    def _strip_username(self, _key, value: str | None):
        return value.strip() if isinstance(value, str) else value

    @validates("phone")
    def _validate_phone(self, _key, value: str | None):
        if value is None:
            return value
        v = value.strip()
    ...
    def lock_user(self, user_id: int | None = None) -> None:
    ...
    def get_last_login_delta(self) -> timedelta | None:
    ...
    def transfer_company(self, company_id: int | None) -> None:
    ...
    def revoke_other_sessions(self, keep_session_id: int | None) -> int:
        n = 0
        for s in list(self.sessions or []):
    ...
    def get_current_otp(self, purpose: str = "login") -> OTPCode | None:
    ...
    def audit(self, action: str, meta: dict | None = None) -> None:
    ...
    # ---------------- Highload locks ----------------
    @classmethod
    def locked_by_id(cls, session: Session, user_id: int) -> User | None:
    ...
    # ---------------- Query helpers (sync) ----------------
    @classmethod
    def find_by_identifier(cls, session: Session, identifier: str) -> User | None:
        if not identifier:
            return None
    ...
    @classmethod
    async def a_find_by_identifier(cls, session, identifier: str) -> User | None:
        if not identifier:
            return None
        ident = identifier.strip()
        q = (
    ...
    @classmethod
    async def a_locked_by_id(cls, session, user_id: int) -> User | None:
