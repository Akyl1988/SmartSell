...
class AuditMixin:
    ...
    last_modified_by: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
...
class LockableMixin:
    ...
    locked_by: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
...
def get_by_id(session: Session, model: type[T], obj_id: Any) -> T | None:
...
def first(session: Session, model: type[T], **kwargs) -> T | None:
...
async def aget_by_id(session: AsyncSession, model: type[T], obj_id: Any) -> T | None:  # type: ignore[valid-type]
...
async def afirst(session: AsyncSession, model: type[T], **kwargs) -> T | None:  # type: ignore[valid-type]
    q = select(model).filter_by(**kwargs).limit(1)
    res = await session.execute(q)
