...
_BULK_LOGGER: logging.Logger | None = None
...
class Category(LenientInitMixin, BaseModel):
    ...
    @validates("slug")
    def _norm_slug(self, _k: str, v: str | None) -> str | None:
        if v is None:
            return None
        vv = v.strip().lower()
        if not vv or " " in vv:
    ...
    @validates("name")
    def _norm_name(self, _k: str, v: str | None) -> str:
    ...
    @validates("parent_id")
    def _validate_parent(self, _k: str, v: int | None) -> int | None:
    ...
    def set_parent(self, session: Session, new_parent_id: int | None) -> None:
...
class Product(LenientInitMixin, VersionedMixin, BaseModel):
    ...
    # --- Kaspi ---
    kaspi_product_id: Mapped[str | None] = mapped_column(String(64), index=True)
    kaspi_status: Mapped[str | None] = mapped_column(String(32), index=True)

    # --- Расширяемое JSON-хранилище ---
    extra: Mapped[str | None] = mapped_column(Text)  # свободный JSON (repricing/preorder и т.д.)
    ...
    # relationships
    company: Mapped[Any | None] = relationship("Company", back_populates="products")
    ...
    def _norm_required(self, key: str, v: str | None) -> str | None:
        if v is None:
            return None
        vv = v.strip()
    ...
    def _validate_stock(self, key: str, value: int | None) -> int | None:
        if value is not None and value < 0:
            raise ValueError(f"{key} cannot be negative")
        return value
    ...
    @classmethod
    def get_all(cls, session: Session, company_id: int | None = None) -> list[Product]:
        query = session.query(cls)
        if company_id is not None:
            query = query.filter_by(company_id=company_id)
        return query.all()
    ...
    def emit_audit(self, action: str, details: dict[str, Any], actor_id: int | None = None) -> None:
    ...
    def reserve(self, qty: int, *, actor_id: int | None = None) -> None:
        if qty <= 0:
            raise ValueError("qty must be positive")
    ...
    def enable_preorder(
        ...
        *,
        ...
        note: str | None = None,
        show_zero_stock: bool | None = None,
    ) -> None:
        """
        Включает предзаказ.
    ...
    def sync_prices_from_variants(
        self, session: Session | None = None, *, commit: bool = False
    ...
    @staticmethod
    def _make_unique_slug(session: Session, base_slug: str, company_id: int | None) -> str:
    ...
    def clone_shallow(self, session: Session | None = None) -> Product:
...
class ProductVariant(LenientInitMixin, VersionedMixin, BaseModel):
    ...
    @validates("sku", "name")
    def _norm_required(self, key: str, v: str | None) -> str:
        vv = (v or "").strip()
    ...
    def _validate_stock(self, _key: str, value: int | None) -> int | None:
        if value is not None and value < 0:
    ...
    @classmethod
    def bulk_restore(cls, session: Session, product_id: int, ids: list[int] | None = None) -> int:
