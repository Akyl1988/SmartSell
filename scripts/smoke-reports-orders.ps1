<#
Smoke: orders CSV report

Usage:
  pwsh -NoProfile -File .\scripts\smoke-reports-orders.ps1 -BaseUrl http://127.0.0.1:8000 -Limit 5
  pwsh -NoProfile -File .\scripts\smoke-reports-orders.ps1 -CompanyId 1001 -Limit 10
...
$url = "$BaseUrl/api/v1/reports/orders.csv"
if ($qs) { $url = "$($url)?$qs" }
