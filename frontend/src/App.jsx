/**
 * frontend/src/App.jsx — SmartSell UI (кампании) — полный файл.
 * Современный, расширяемый каркас:
 * - Health-индикатор
 * - Список кампаний (/api/v1/campaigns?skip&limit)
 * - Поиск (клиентский)
 * - Создание/редактирование/удаление
 * - Быстрая смена статуса
 * - Пагинация
 * - Отлов и показ ошибок
 *
 * Зависимости: apiClient (см. frontend/src/api/api.js)
 */
...
function CreateEditDialog({ open, onClose, onSubmit, busy, initial }) {
  ...
  const [recipient, setRecipient] = useState(initial?.messages?.[0]?.recipient ?? '')
  const [content, setContent] = useState(initial?.messages?.[0]?.content ?? '')
...
export default function App() {
      ...
      try {
        ...
        const tot =
          typeof res?.total === 'number'
            ? res.total
            : Array.isArray(res)
            ? skip + arr.length + (arr.length === ps ? ps : 0)
            : arr.length
  ...
  async function handleCreate(payload) {
    ...
    try {
      ...
      await loadList({ page: 0, pageSize })
      setDialogOpen(false)
      setEditItem(null)
      setError(null)
      setPage(0)
  ...
  async function handleDelete(c) {
    if (!c?.id) return
    const ok = window.confirm(`Delete campaign "${toDisplayName(c)}"?`)
    if (!ok) return
    try {
